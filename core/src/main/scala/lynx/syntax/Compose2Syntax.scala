...
trait Compose2Syntax {
  private type T[A, B <: A] = PartialFunction[(A, A), B]

  private def tag[F[_] : Sync, A, B <: A](pf: T[A, B]) = Tagged2[F, A](map[F, A, A, (A, A)](pf))

  implicit class ComposeOps2[F[_], A, B <: A](tags: Tags[F, A]) {
...
object Compose2Syntax extends Compose1Syntax
