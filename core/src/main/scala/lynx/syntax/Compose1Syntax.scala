...
trait Compose1Syntax {
  private type T[A, B <: A] = PartialFunction[A, B]

  private def tag[F[_] : Sync, A, B <: A](pf: T[A, B]) = Tagged1[F, A](map[F, A, A, A](pf))

  implicit class ComposeOps1[F[_], A, B <: A](tags: Tags[F, A]) {
...
object Compose1Syntax extends Compose1Syntax
