...
import lynx.types.{PartialFunction1, PartialFunction2, PartialFunctionN, Tags}

trait ComposeF2Syntax {
  private type T[F[_], A] = PartialFunction2[F, A]

  private def tag[F[_] : Functor, A, B <: A](pf: T[F, A]) = Tagged2[F, A](mapF[F, A, A, (A, A)](pf))

  implicit class ComposeOpsF2[F[_], A, B <: A](tags: Tags[F, A]) {
